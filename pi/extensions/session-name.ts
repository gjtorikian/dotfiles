...
function loadConfig(): SessionNameConfig {
  try {
    // Resolved next to this file, not under $HOME: ~/.pi/agent symlinks
    // individual entries into the dotfiles repo, so a homedir path would read
    // a stale copy outside it.
    const path = fileURLToPath(new URL("./session-name.json", import.meta.url));
